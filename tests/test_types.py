...
def test_the_catalogue_says_who_uses_what():
    ...
    # And channels and messages too, since Chat is a Quill.
    assert "channel" not in rows and "message" not in rows
...
def test_the_types_are_on_the_api(client, auth):
    ...
    # Boards, tasks, calendars, events, channels and messages are datamodels
    # in the record store now, not here.
    assert "task" not in by_key and "event" not in by_key and "message" not in by_key
