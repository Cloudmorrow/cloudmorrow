...
async def test_secrets_is_a_card_of_its_own(app):
    ...
    async with app.run_test(size=(120, 34)) as pilot:
        ...
        # footer does not repeat them; the Quills in the catalog's order,
        # every one that was built in keeping the key it had. Nothing is
        # built in any more, so there is no YOUR CLOUD section.
        cards = [
        ...
        assert not screen.query_one("#nav-section-cloud").display
